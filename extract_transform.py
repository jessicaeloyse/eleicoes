...
def download_arquivo_to_df(ano):
    ...
    nome_arquivo_full = f"{pasta}/{nome_arquivo}.csv"
    url = f"https://cdn.tse.jus.br/estatistica/sead/odsele/votacao_secao/{nome_arquivo}.zip"

    if (
        not os.path.exists(nome_arquivo_full)
        or os.path.getctime(nome_arquivo_full) <= data_min
    ):
        ...
        logging.debug(f"Arquivo {nome_arquivo_full.split('/')[-1]} já existe")
...
def transformacoes(df):
    ...
    logging.debug("Tratando as regiões")
...
def agrupamentos(df_turno):
    df_turno = df_turno.groupby(
        [
            "SG_UF",
            "NM_UE",
            "NM_MUNICIPIO",
    ...
    for col in "NM_MUNICIPIO", "SG_UF", "REGIAO":
        df_vencedor = df_turno.groupby([col, "NM_VOTAVEL"], as_index=False)[
            "QT_VOTOS"
        ].sum()

        df_vencedor["RN"] = (
            df_vencedor.sort_values("QT_VOTOS", ascending=[False])
            .groupby([col])
            .cumcount()
            + 1
        )
        ...
        df_vencedor = df_vencedor.rename(
            columns={
                "NM_VOTAVEL": f"VENCEDOR_{col.split('_')[-1]}",
                "QT_VOTOS": f"VENCEDOR_VOTOS_POR_{col.split('_')[-1]}",
            }
        )
